...
export default function EthDirectoryPage() {
              ...
              <AccordionContent className="pt-4 pb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {deptData.data?.map((person: any) => {
                    ...
                    const imageUrl = attr.image ? `https://ethz.ch${attr.image}` : null;
                      ...
                      <Card key={attr.id} className="overflow-hidden">
                        <CardHeader className="flex flex-row items-center gap-4 pb-2">
                          <div className="relative flex h-[200px] w-[140px] shrink-0 items-center justify-center overflow-hidden rounded-md border bg-muted">
                            {imageUrl ? (
                              // Using <img> avoids occasional Turbopack dev manifest issues with next/image.
                              <img
                                src={imageUrl}
                                alt={attr.name}
                          ...
                          <div className="flex flex-col">
                            <h3 className="font-semibold leading-tight">{attr.name}</h3>
                          </div>
                        </CardHeader>
